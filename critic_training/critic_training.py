...
MODEL_NAME = "models/biggest/rover_model_01_ver01_epoch0009_val_loss0.0005.h5"
MODEL_PATH = '/media/usafa/data/models/critics/'
...
def define_model(input_shape=(120, 200, 1)):
    ...
    controls = Input(shape=(2,))

    x = Conv2D(32, (2, 2), activation='relu')(image)
    ...
    x = Flatten()(x)
    x = Concatenate()([x, controls]) # Add controls to the model
    ...
    model = Model(inputs=[image, controls], outputs=out)
...
def train_model(amt_data=1.0):
    ...
    with tf.device(DEVICE):
        ...
        model = define_model(input_shape=(120, 200, 1))
        # model = get_model(MODEL_NAME)
        model.summary()  # Print a summary of the model architecture
        
        # Path for saving the best model checkpoints
        filePath = MODEL_PATH + "critic_model_" + f"{MODEL_NUM:02d}_ver{TRAINING_VER:02d}" + "_epoch{epoch:04d}_val_loss{val_loss:.4f}.h5"
...
def main():
    ...
    history = train_model(amt_data=1)
